...
async function storePsaScansForCard(db, bucket, userId, cardId, certNumber) {
  try {
    ...
    console.warn("[execute-trade] PSA image store failed:", err.message);
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Best-effort PSA official-scan storage for each received card. Runs
    // AFTER the trade has committed and is fully non-fatal — a scan
    // fetch/store failure must never surface as a trade error. Uniform-path
    // (ROADMAP): fetch by cert server-side for each received card, keyed
    // under the new card's own prefix.
    const bucket = process.env.CARD_IMAGES_BUCKET;
    await Promise.all(
      receivedCardIds.map((rc) =>
        storePsaScansForCard(db, bucket, userId, rc.id, rc.certNumber?.trim())
      )
    );
